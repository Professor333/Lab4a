...
qqPlot(out11$resid,ylab="Z value") #NPP test, looks pretty normal, but there is some slight tailing at the ends
shapiro.test(out11$resid) #Results indicate that the data is normal

par(mfrow = c(2, 2))  
plot(out11) #Everything looks about normal, but scale-location looks a little non-linear. That test assumes homoscedasticity, and if the assumption is met, then the line is flat, but if it is not met, then the line will be curved some way.
